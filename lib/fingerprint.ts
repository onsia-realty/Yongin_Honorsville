...
export async function generateFingerprint(): Promise<FingerprintData> {
  ...
  const components = [
    canvasHash, webglHash, audioHash, fontsHash,
    timezone, language, colorDepth.toString(), pixelRatio.toString(),
    hardwareConcurrency.toString(), deviceMemory?.toString() || '',
    device.screenWidth.toString(), device.screenHeight.toString(),
    device.browser, device.os, touchSupport.toString(),
  ].join('|');
  ...
  return {
    ...
    canvasHash, webglHash, audioHash, fontsHash,
    timezone, language, languages, colorDepth, pixelRatio,
    hardwareConcurrency, deviceMemory, touchSupport, cookiesEnabled, doNotTrack,
  };
}
